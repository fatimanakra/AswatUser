...
export class AmeriqueComponent implements OnInit, OnDestroy {
  ...
  checkAndArchiveExpiredPosts(): void {
    ...
    this.ameriquePosts.forEach(post => {
    ...
    if (newPost.typeName === 'first') {
      if (this.currentFirstPost && new Date(this.currentFirstPost.expirationDate) <= now) {
        this.moveCurrentFirstPostToArchive().subscribe(() => {
          ...
          this.ameriquePosts.unshift(newPost);
        });
  ...
  reorderPostsByType(typeName: string, limit: number, service: any, createPlaceholder: () => PostDTO): void {
    const posts = this.ameriquePosts.filter(post => post.typeName === typeName && !post.archived && post.approved);
    ...
    this.ameriquePosts = [...this.ameriquePosts.filter(post => post.typeName !== typeName), ...posts];
  }
  

  createPlaceholderReportsPost(): PostDTO {
    return this.createPlaceholderPost('report','amerique');
  }

  createPlaceholderLatestPost(): PostDTO {
    return this.createPlaceholderPost('latest','amerique');
  }

  createPlaceholderTrendPost(): PostDTO {
    return this.createPlaceholderPost('trend','amerique');
  }
  ...
  getMarocPosts(): void {
    this.latestService.getApprovedPostsByCategory('amerique')
      .subscribe(posts => {
  ...
  getFirstByType(typeName: string): PostDTO[] {
    return this.ameriquePosts.filter(post => post.typeName === typeName && !post.archived).slice(0, 1);
  }



  getFirstThreePostsByType(typeName: string): PostDTO[] {
    return this.ameriquePosts.filter(post => post.typeName === typeName && !post.archived).slice(0, 3);
  }

  getFirstTwoPostsByType(typeName: string): PostDTO[] {
    return this.ameriquePosts.filter(post => post.typeName === typeName && !post.archived).slice(0, 2);
  }

  getFirstFourPostsByType(typeName: string): PostDTO[] {
    return this.ameriquePosts.filter(post => post.typeName === typeName && !post.archived).slice(0, 4);
  }
}
